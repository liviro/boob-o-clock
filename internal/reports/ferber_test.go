...
func TestSuggestFerberNight(t *testing.T) {
	...
	nonFerber := &domain.Session{Kind: domain.SessionKindNight, FerberEnabled: false}
	if got := SuggestFerberNight(nonFerber); got != nil {
		t.Errorf("non-Ferber → want nil, got %v", *got)
	}
	n := 4
	ferber := &domain.Session{Kind: domain.SessionKindNight, FerberEnabled: true, FerberNightNumber: &n}
	...
	enabledNoNumber := &domain.Session{Kind: domain.SessionKindNight, FerberEnabled: true}
