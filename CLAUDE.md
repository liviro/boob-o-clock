...
- **Feature flags for sleep-training modes**: runtime-toggleable via env var on the Go binary (e.g. `FERBER_ENABLED=true`), plumbed through `api.Config` in `cmd/server/main.go`. Surfaced to the frontend via `GET /api/config` → `{features:{...}}`, fetched once on boot through `ConfigProvider` in `web/src/hooks/useConfig.tsx`. The server enforces flag-off by rejecting opt-in requests (e.g. `POST /api/session/start` with `ferber:{}`) and by suppressing mode-specific response fields like `suggestFerberNight`; the client gates all mode-specific UI (Start-Night toggle, chart highlights, badges, detail blocks) via `useConfig().features.<mode>`. In-progress sessions from before a flag flip continue to render normally — the flag controls new opt-ins, not stored history. To add a new mode, add a `Bool` to `api.Config`, surface it in `configFeatures`, and gate the relevant UI behind `useConfig().features.<newMode>`.
