...
- **Ferber is per-night, opt-in**: the `nights` row carries `ferber_enabled` + `ferber_night_number`. The backend picks the action set (`SelectActionsForNight` in `reports/`) and derives the live-session fields (`CurrentFerberSession` in `reports/`) so the frontend never branches on Ferber state.
